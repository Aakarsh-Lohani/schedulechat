...
export async function POST(_req: Request, { params }: { params: { id: string } }) {
  ...
  const contributedSeconds = session.status === "running" ? Math.max(0, Math.round(rawElapsedSeconds - COUNTDOWN_SECONDS)) : 0;
  ...
  return NextResponse.json({ session: { id: String(session._id), status: session.status, contributedSeconds } });
}
